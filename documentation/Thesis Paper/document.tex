...
%Possibly move to after the Case study explaination.
...
\\\\
As mentioned in section \ref{sec:idebuild}, we initially started out with a project made in Netbeans using Java EE serving JSP pages. This was a monolithic application that included client, business logic and database. This was our first experience with Enterprise java development. We used a locally hosted Payara server which we used to deploy our enterprise application archive. A simple SQL datastructure was running on an external MySQL server, which can been seen on figure \ref{fig:sqldiagram} %insert picture of original schematic.
\begin{figure}[h!]
	\centering
	%\includegraphics[width=0.75\linewidth]{usecase.jpg}
	\caption{Original MySQL datastructure: This image shows the prototype relational model we used while working with a single SQL database}
...
% TODO Configure all separate project and make them work with spring boot
% disadvantage, the initial migration takes quite some time, but a lot of information becomes more similar because of the simplification of the modules.
% disadvantage, combining technologies, especially security. security by itself is not difficult but combining it with microservices becomes more complex.
% exception handling when a service is not running.
% Test if the application server works with these new services
% Retrieving data is now done using JSON object, but posts are still entities.
\\\\
wip
